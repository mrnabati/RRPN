...
class NuScenes:
    ...
    ## ADDED BY RAMIN NABATI -------------------------------------------------------
    def get_select_sample_data(self, sample_data_token, box_vis_level=BoxVisibility.ANY,
                               selected_anntokens=None, min_ann_vis_level: int=0) -> \
            Tuple[str, List[Box], np.array]:
        """
        Returns the data path as well as all annotations related to that sample_data.
        Note that the boxes are transformed into the current sensor's coordinate frame.
        ...
        ## Filter anns based on visibility
        if min_ann_vis_level > 0:
            selected_anntokens = []
            sample_rec = self.get('sample', sd_record['sample_token'])
            for ann_token in sample_rec['anns']:
                ann = self.get('sample_annotation', ann_token)
                vis_token = ann['visibility_token']
                if int(vis_token) >= min_ann_vis_level:
                    selected_anntokens.append(ann['token'])

        # Retrieve all sample annotations and map to sensor coordinate system.
        boxes = self.get_select_boxes(sample_data_token, selected_anntokens)
    ...
    def get_select_boxes(self, sample_data_token: str,
                         sample_annotations: List=None) -> List[Box]:
        """
        Instantiates Boxes for select annotations for a particular sample_data record.
        ...
        is applied to estimate the location of the boxes at the time the sample_data
        was captured.
        :param sample_data_token: Unique sample_data identifier.
        """
        ...
        if curr_sample_record['prev'] == "" or sd_record['is_key_frame']:
            # If no previous annotations available, or if sample_data is keyframe just return the current ones.
            boxes = list(map(self.get_box, sample_annotations))

        else:
            prev_sample_record = self.get('sample', curr_sample_record['prev'])

            if sample_annotations is not None:
                curr_ann_recs = [self.get('sample_annotation', token) for token in sample_annotations]
                prev_ann_recs = [self.get('sample_annotation', token) for token in prev_sample_record['anns']]
            else:
                curr_ann_recs = [self.get('sample_annotation', token) for token in curr_sample_record['anns']]
                prev_ann_recs = [self.get('sample_annotation', token) for token in prev_sample_record['anns']]
